"""
app.py - Painel de metricas restrito a administradores.
"""
...
st.title("Painel de Metricas")
st.caption(f"Dados dos ultimos {periodo} dias - comparado ao periodo anterior")
st.divider()
...
try:
    ...
    col1.metric(
        "Receita Total",
        f"R$ {kpis['receita']:,.2f}",
        f"{kpis['receita_delta']:+.1f}% vs periodo anterior",
    )
    col2.metric(
        "Total de Pedidos",
        f"{kpis['total_pedidos']:,}",
        f"{kpis['pedidos_delta']:+.1f}% vs periodo anterior",
    )
    col3.metric(
        "Ticket Medio",
        f"R$ {kpis['ticket_medio']:,.2f}",
        f"{kpis['ticket_delta']:+.1f}% vs periodo anterior",
    )
